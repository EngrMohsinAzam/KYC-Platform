...
import { getNetworkInfo, isMetaMaskInstalled, checkKYCStatus } from '@/app/(public)/wallet/web3'
import { checkStatusByWallet } from '@/app/api/api'
import { isMobileDevice, openMetaMaskMobile, getMobileWalletDeepLink } from '@/app/(public)/wallet/mobile-wallet'
...
export default function ConnectWallet() {
  ...
  const proceedWithConnection = async (walletAddress?: string) => {
    try {
      ...
      if (!walletAddress) {
        ...
        if (!walletAddress) {
          ...
          if (isMobile && !window.ethereum) {
            ...
            // Don't return - let it try to connect when ethereum becomes available
            ...
            if (window.ethereum) {
              try {
                ...
                console.error('Error connecting after wait:', err)
              }
            }
            // If still no wallet address after waiting, return and let useEffect handle it
            if (!walletAddress) {
              return
            }
          }
          ...
          if (!isMobile || window.ethereum) {
            const { connectWallet } = await import('@/app/(public)/wallet/web3')
            ...
            // On mobile, if ethereum still not available, just return and wait
            return
          }
        }
      }
      ...
      const isMobile = isMobileDevice()
      if (isMobile) {
        alert(error.message || 'Failed to connect wallet. Please make sure MetaMask app is installed and try again.')
      } else {
