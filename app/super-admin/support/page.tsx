...
export default function SuperAdminSupportPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.log('📡 [Support Issues] Fetching:', url, 'Status filter:', status)
      
      const response = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` },
      })
      
      console.log('📥 [Support Issues] Response status:', response.status)
      const data = await response.json()
      console.log('📥 [Support Issues] Full response:', JSON.stringify(data, null, 2))
      
      if (!data.success) {
        console.error('❌ [Support Issues] API returned success: false', data.message)
      ...
      console.log('📋 [Support Issues] Parsed list:', list)
      console.log('📋 [Support Issues] List length:', list.length)
      console.log('📋 [Support Issues] Is array:', Array.isArray(list))
      
      setIssues(Array.isArray(list) ? list : [])
    } catch (e: any) {
      console.error('❌ [Support Issues] Error:', e)
    ...
    <main className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">View support issues from users.</p>
        </div>
              ...
              issues.map((i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => id && router.push(`/super-admin/support/${id}`)}
