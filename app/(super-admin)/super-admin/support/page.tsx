...
import { getSuperAdminToken } from '@/app/api/super-admin-api'
...
export default function SuperAdminSupportPage() {
  ...
  const load = async () => {
    ...
    try {
      setLoading(true)
      setError('')
      const qs = new URLSearchParams({ page: '1', limit: '50', ...(status !== 'all' ? { status } : {}) })
      const response = await fetch(`/api/super-admin/support/issues?${qs.toString()}`, {
    ...
    <main className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">View all support issues across the platform.</p>
        </div>
